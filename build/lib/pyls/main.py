...
def print_items(items, long_format=False):
    ...
    Args:
    items (list): List of dictionaries representing items to print.
    long_format (bool): If True, prints additional information.
    """
    if long_format:
...
def navigate_to_path(directory, path):
    ...
    Args:
    directory (dict): The root directory structure.
    path (str): The path to navigate to.

    Returns:
    list: List of items in the target directory or the target file itself if path is a file.
